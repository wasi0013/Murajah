/**
 * Murajah Indopak Data Loader Utility
 * Handles loading and parsing Quran data from JSON files for Indopak layout (610 pages)
 */
...
const fetchAndCacheResource = async (resourceConfig, murajahDB) => {
  Logger.log(`[Murajah] Fetching ${resourceConfig.key} from ${resourceConfig.url}...`);
  ...
  Logger.log(`[Murajah] Fetch complete for ${resourceConfig.key}, parsing JSON...`);

  const data = await response.json();
  Logger.log(`[Murajah] JSON parsed for ${resourceConfig.key}, entries: ${typeof data === 'object' ? Object.keys(data).length : 'N/A'}`);
  ...
  if (murajahDB) {
    try {
      ...
      if (dataSize < 5000000) {
        Logger.log(`[Murajah] Caching ${resourceConfig.key} to IndexedDB (${(dataSize/1024/1024).toFixed(2)}MB)...`);
        await murajahDB.saveCachedResource(cacheRecord);
        Logger.log(`[Murajah] Cached ${resourceConfig.key} successfully`);
      } else {
        Logger.log(`[Murajah] Skipping cache for ${resourceConfig.key} - too large (${(dataSize/1024/1024).toFixed(2)}MB)`);
      }
    } catch (cacheError) {
...
const loadResourceWithCache = async ({ resourceConfig, murajahDB, onBackgroundUpdate }) => {
  ...
  Logger.log(`[Murajah] Loading resource: ${resourceConfig.key}`);
  
  if (murajahDB) {
    try {
      Logger.log(`[Murajah] Checking cache for ${resourceConfig.key}...`);
      const cached = await murajahDB.loadCachedResource(resourceConfig.cacheId);
      Logger.log(`[Murajah] Cache result for ${resourceConfig.key}:`, cached ? 'found' : 'not found');
      if (cached?.data) {
        Logger.log(`[Murajah] Using cached ${resourceConfig.key}, entries: ${typeof cached.data === 'object' ? Object.keys(cached.data).length : 'N/A'}`);
        scheduleResourceRefresh(resourceConfig, murajahDB, onBackgroundUpdate);
        return cached.data;
      }
      Logger.log(`[Murajah] No valid cache data for ${resourceConfig.key}`);
    } catch (error) {
...
/**
 * Load all Quran data files in parallel (Indopak layout)
...
export const loadAllQuranData = async ({ murajahDB, onTranslationUpdate, onResourceUpdate } = {}) => {
  ...
  if (dataCache.isLoaded) {
    ...
    Logger.debug(Logger.MODULES.DATA, 'Returning cached Indopak Quran data');
    return {
  ...
  try {
    Logger.info(Logger.MODULES.DATA, 'Loading Indopak Quran data files...');
    ...
    Logger.info(Logger.MODULES.DATA, 'Indopak Quran data loaded successfully', {
    ...
    Logger.error(Logger.MODULES.DATA, 'Failed to load Indopak Quran data', error);
    throw error;
  }
};

/**
 * Get text for a specific page (Indopak layout: 1-610)
 * @param {number} pageNum - Page number (1-610)
...
/**
 * Get page text with word-by-word breakdown (Indopak layout)
...
export const clearDataCache = () => {
  ...
  Logger.info(Logger.MODULES.DATA, 'Indopak data cache cleared');
};
