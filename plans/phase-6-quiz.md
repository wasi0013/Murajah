...
- [ ] **6.0.2** Candidate + distractor pools. `buildCandidatePool(scopePages, layout)` → the flat list of `{ surah, ayah, page }` eligible for questions (all verses whose words fall on a scope page). Distractors are drawn **from the same pool** (other verses / other words on scope pages) — plausible near-neighbours, and no full-corpus scan (fixes **B7**). Falls back to widening beyond scope only if the pool is too small for N options.
...
- [ ] **6.0.3** *Decision record (no task):* **runtime page-chunk assembly, not a build-time quiz dataset.** Rationale: the chosen page-scoped model means candidate pages are few and already cached by the reader; continuation needs at most one adjacent page; translation is a surah-chunk the app loads anyway. A pre-baked `data/quiz/*` dataset would duplicate content already shipped and add a build step for no runtime win. **Fallback trigger:** if profiling on a low-end device shows the adjacent-page loads janking continuation, revisit with a per-surah verse-index dataset (`{ "s:a": { page } }`) — content stays in page chunks, only the ayah→page index is pre-baked.
...
- [ ] **6.2.1** `stores/quiz.ts` (Pinia) — session state (current mode, scope selection, current question, streak, session score/attempts) + the durable per-page accuracy tally. Durable shape: `Map<page, { correct: number; total: number }>`; accuracy = `correct / total` (0–1), the exact input the weakness scorer wants.
  - *Design point (record in code):* raw cumulative ratio means a page quizzed well long ago stays "good" forever. Ship a **windowed** tally (cap `total`, decay old counts, or keep last-K per page) so a page that's started failing recovers a weak signal. Keep the interface `page → 0..1` regardless of the internal window.
- [ ] **6.2.2** Persist under a **new `quiz` key in the existing `murajah-userdata` DB** (not a separate database — that was **B5**). `serializeQuizAccuracy` / `deserialize` rebuild plain objects (the Phase-4/5 proxy-safe gotcha). `useQuizPersistence` composable mirrors the Phase-5 pattern: debounced deep-watch save + `hydrate()` + `dispose()`.
...
- [ ] **6.3.1** Feed per-page quiz accuracy into `generateDailyTasks`. Replace the stubbed `quizScores: input.quizScores ?? new Map()` path so `useToday` passes `quiz.accuracyByPage` through `planBuilder` → `calculateAllWeaknesses`. Verse-level answers roll up to pages via the verse's `page` (already known from sourcing).
