...
- [x] **3.4.1** Layout `SegmentedControl` (Phase 2) bound to `reader.setLayout`; Indopak uses its single font family + Indopak reading metrics (`--indopak-line-height`/`--indopak-tracking`); QPC uses per-page glyph family + QPC metrics. `ReadingSurface` already renders both — feed it the resolved family + per-layout metrics.
  ...
  - **Follow-up (Indopak fit + spacing, user-reported):** Indopak uses one Nastaleeq font (not QPC's per-page glyph fonts), so line widths vary and were overflowing/clipping on phones and over-stretching short lines. Fixes: (a) `--qpc-line-height` 1.7→2.0 to match Indopak's airier vertical rhythm; (b) Indopak lines switched from full `space-between` to centred with a tight fixed `column-gap` (0.28em) so word spacing is consistent (~halved), not stretched; (c) **shrink-to-fit** in `ReadingSurface` (`fitIndopakLines`, rect-union measurement since centred overflow defeats `scrollWidth`; ResizeObserver + prop watchers, floored at 55%) so every Indopak line fits the column → **exactly 15 lines, no wrap/overflow** at any width. WBW still wraps. e2e locks 15 lines + zero overflow on a 390px phone.
