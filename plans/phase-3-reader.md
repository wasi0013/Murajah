...
- [x] **3.4.1** Layout `SegmentedControl` (Phase 2) bound to `reader.setLayout`; Indopak uses its single font family + Indopak reading metrics (`--indopak-line-height`/`--indopak-tracking`); QPC uses per-page glyph family + QPC metrics. `ReadingSurface` already renders both — feed it the resolved family + per-layout metrics.
  ...
  - **Follow-up (Indopak fidelity, user-reported — sacred-text accuracy):** Indopak uses one Nastaleeq font (not QPC's per-page glyph fonts), so the 15 authentic mushaf lines (word grouping from `indopak-15-lines.json`) render at varying natural widths (~30% spread), which overflowed/clipped and over-stretched short lines. Final approach in `ReadingSurface.fitIndopakLines`: **scale each non-WBW line so its content fills the column width** (rect-union measurement; clamp 0.5–1.15 so an outlier line stays near its neighbours' size; `space-between` absorbs sub-pixel residual; ResizeObserver + prop watchers). Result: **exactly 15 lines, each justified edge-to-edge like a printed mushaf**, at any width, no wrap/clip. Also `--qpc-line-height` 1.7→2.0 (airier, matching Indopak). WBW still wraps. e2e locks 15 lines, zero overflow, and ≥97% line-fill on a 390px phone.
...
- [x] **3.6.1** WBW `Toggle` (+ lang en/bn): when on, render each word's translation beneath it, lazy-loading the **per-surah** `TranslationChunk` for the page's surah(s) through `DataClient.getTranslations`. Layout: stack translation under the Arabic word without breaking line justification.
  ...
  - **Follow-up (page width):** the reading surface now has a centred `max-width: 43rem` column so the authentic `space-between` line justification reads like a real mushaf page on wide desktops instead of stretching full-bleed, with a touch more inter-word breathing room in normal reading (both user-reported). Phones (narrower) unaffected; the desktop 2-up spread is still Phase 3b.
  - **Follow-up (WBW 15-line structure):** glosses are wider than their word, so a mushaf line can't fit 15-per-column on one row at a readable size — forcing it would either shrink text below readability or require per-line horizontal scrolling. Decision (with user): **keep wrapping** (standard for WBW readers) but add a subtle hairline + spacing between the 15 mushaf-line blocks so the page structure stays perceivable even when a line wraps.
...
- [x] **3.7.1** Tapping a word opens a **`Popover`** (Phase 2) with its morphology (root, lemma, POS, grammar) from the per-surah morphology chunk (3.0.2/3.0.3), **lazy code-split** so the morphology view + its data never enter the initial reader bundle. Popover anchors to the word, RTL-correct, keyboard-dismissible.
  ...
  - **Done:** taps detected in the pager (a pointer-up that never became a drag → `closest('[data-loc]')`, read mode only). `useMorphology` (per-surah cache, stale-fetch guarded) drives a **`defineAsyncComponent`** `MorphologyPopup.vue` (its own 1.22KB-gz chunk + data, absent from the reader entry) — anchored to the word, teleported, `useFocusTrap`, Escape/outside-click/paging close; renders the trusted per-word analysis HTML (`.at` Arabic RTL / `.ab` italic, faithful to legacy). 5 unit tests (load/cache/empty/close/stale-race) + e2e (tap→popup, chunk loads on demand not upfront, content, dismiss).
- [x] **3.7.2** Word "selected"/"morphology-active" states use the existing `ReadingSurface` state classes; only one active at a time; tap-again/elsewhere dismisses.
  - *Verify:* active word shows the `state-morphology`/`state-selected` style in all themes; dismiss clears it.
  - **Done:** the pager passes `wordStates = { [activeLocation]: 'morphology' }` to the current surface only, so exactly one word carries `state-morphology`; tapping another word switches it (outside-click ignores word taps), and Escape/paging clears it. e2e asserts the single highlight appears and clears.

## 3.8 — Mistake-mark mode
- [x] **3.8.1** Mode switch (`read` ↔ `mark-mistake`) via Phase 2 control. In mark mode, tapping a word toggles its **mistake** state (instead of opening morphology); the word gets the `state-mistake` style. Mistakes persist to IndexedDB (per word location), keyed compatibly with legacy so **migrated data shows up and new marks round-trip**.
  ...
  - **Done:** Read/Mark `SegmentedControl` drives `reader.mode`; the pager's tap is mode-gated (mark → `useMistakes.markWord`, read → morphology). **Key finding:** word ids are location-stable across QPC/Indopak (all 83,668 verified), so the canonical model is legacy-exact `Map<qpcPage, Set<wordId>>` while **display uses the global id-union** (`mistakeIds`) — marks show in both scripts regardless of page numbering. Marking keys by the real QPC page (current page in QPC; resolved via the QPC nav index in Indopak). Persisted to a dedicated `murajah-userdata` store (`core/storage/userData.ts`, legacy `{page:[ids]}` shape), debounced. `state-mistake` styling from Phase 2 (2.7.2, AA-verified). e2e: mark→reload persists→unmark→read-mode-morphology. 7 unit tests.
- [x] **3.8.2** Mistakes feed the memorization layer without duplicating it: the reader **reads/writes the same store** the Phase 4 weakness/hasanah logic will consume (no parallel schema). Confirm no regression against migrated legacy mistakes.
  ...
  - **Done:** the store's `snapshot()` is the exact `Map<page, Set<wordId>>` `calculateAllWeaknesses` consumes; unit test imports a legacy v2.0.0 export via `parseLegacyExport` → `store.setAll` → asserts the words show (global membership) and `weaknessScorer` scores the page unchanged (set size = mistakeCount). No parallel schema; `core/storage/userData.ts` is the Phase-4-shared user-data store (mistakes now, memorized/perfect-revisions later).
  - **Note for Phase 4:** legacy `highlightWord` also **decremented `perfectRevisions`** on marking a mistake — intentionally *not* replicated here (perfect revisions land in Phase 4); wire that coupling there.
