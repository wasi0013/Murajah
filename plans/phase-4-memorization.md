...
- [x] **4.2.1** Convert to **`.ts`** with real types: `pageHasanah`, `calculations`, `weaknessScorer`, `scoreFormatter`. **Drop** the legacy `calculateTotalScore = Σ hasanah×perfectRevisions` as the *live* total (it's now the migration-seed only); keep the per-page weight + colour/grid/stat helpers + weakness maths.
  - *Verify:* kept helpers unchanged (characterization tests); `vue-tsc` clean; removed/replaced pieces documented.
  - **Done:** `git mv`'d all four `.js`→`.ts` (history preserved) and added real types: `pageHasanah` (`readonly number[]` + typed `getPageHasanah`), `calculations` (typed helpers + `MistakeBubble`/`MemorizedGridItem`/`Statistics` interfaces; dropped the now-unused `PAGE_HASANAH_VALUES` import to satisfy `noUnusedLocals`), `scoreFormatter` (guards kept via `number|null|undefined` params), `weaknessScorer` (typed params + a `MapLike`/`Countable` read-only accessor preserving the legacy Map-or-object duck-typing). `.js` import specifiers across callers/tests resolve to the `.ts` sources (Vite/bundler resolution) — no caller edits needed. `vue-tsc` clean; build clean. **Documented:** `calculateTotalScore`/`calculatePageScore` **kept but not wired live** — a doc-comment marks them migration-seed only (hasanah is the store's cumulative counter); the legacy Tailwind-class colour helpers (`getScoreColor`, `getStatusColor`, magnitude colours) are **retained for parity but unused** by the token-based Progress UI (comment added).
- [x] **4.2.2** **Parity where it still applies**: the 6-tier strength colours (`getScoreColor`), grid/juz shapes, and `weaknessScorer` maths match legacy on a migrated fixture; the seeded hasanah matches `Σ hasanah×strength`.
  - *Verify:* numbers match on the committed fixture.
  - **Done:** parity is locked by the **unchanged characterization suites** — `calculations.test.js` (getScoreColor 6-tier, `generateMemorizedGrid` juz shapes, `calculatePageScore`/`calculateTotalScore`), `weaknessScorer.test.js` (full maths), `scoreFormatter.test.js` — all still green against the converted `.ts` (identical behaviour). Seeded-hasanah parity (`Σ pageHasanah×strength`) covered by `progress-migration.test.ts`. *(A dedicated committed legacy-export fixture e2e still lands under 4.10.1.)*
...
- [x] **4.3.2** **Interaction**: tap a cell → per-page sheet (memorized toggle · strength stepper · "open in reader"); a bulk-select mode (4.4); keyboard-navigable roving grid.
  ...
  - **Done:** tap → `BottomSheet` (memorized `Toggle` · strength stepper · "Open in reader" → `/read/qpc/{page}`); mark→persist→reload e2e-asserted. Cells are a **roving-tabindex group** — one Tab stop; a single cell holds `tabindex=0`, arrows move focus (Left/Right ±1 page, Up/Down ±one row via a live `gridTemplateColumns` read, Home/End to the ends) and update the roved cell on focus, Enter/Space activate. e2e: ArrowRight/Home/End land on the exact cell, ArrowDown moves by a row, Enter opens the sheet.
...
- [x] **4.6.1** A 30-item juz overview: per-juz memorized progress (bar + %/count) from the store; tapping a juz scrolls/filters the grid. Juz boundaries come from the **derived `nav.juzToPage`** for the active layout (accurate per-layout — *not* the legacy 20-page blocks or the off-by-one hardcoded tables; see [legacy-hardcoded-tables.md](./legacy-hardcoded-tables.md)).
  ...
  - **Done:** each juz section has a labelled `role="progressbar"` (aria-valuemin/now/max) + count; boundaries from `buildJuzGroups(nav.juzToPage('qpc'))`; 30 groups spanning 604 (unit + e2e asserted). A **juz-jump bar** (30 chips, horizontally scrollable) scrolls the grid to any juz (`scrollIntoView`, reduced-motion aware) and arms that juz's first page for the roving group; e2e asserts juz 30 scrolls into the viewport.
...
- [x] **4.8.1** Feed `calculateAllWeaknesses`: `perfectRevisions` (= strength), `mistakesMap`, and **`pageReviewData`** (`{page → {lastReviewDate, reviewCount}}`). Establish review data — a lightweight `lastReviewedAt`/`reviewCount` bumped when a page earns a reading reward or a revision here (full history is Phase 5); quiz accuracy is Phase 6 (neutral until then).
  ...
  - **Done:** store gained `reviewData: Map<page, {lastReviewDate:'YYYY-MM-DD', reviewCount}>` + `markReviewed(page, date?)` (via `todayISODate`); bumped by `recordPerfectRevision` and by `useReadingReward` **once per session** (first reward, not per threshold — tested). Persisted in `StoredProgress.reviewData` (serialize rebuilds **plain** objects — Vue reactive proxies aren't structured-cloneable, which had silently failed the IDB write; caught + fixed with a round-trip test) and hydrated via `setAll`; legacy migration seeds it empty (`reviewData?` optional on disk → back-compatible). `useMemorization` now passes `progress.reviewData` (was neutral `{}`). Field shape matches `weaknessScorer.PageReviewData` so it feeds directly; scorer already unit-tested for recency/low-review with a fixed `today`. 431 unit + 55 e2e green.
...
- [x] **4.10.2** **A11y**: keyboard-navigable labelled grid (each cell announces page + status); colour never the only signal; axe clean across light/dark/sepia.
  ...
  - **Done:** axe WCAG2 A/AA clean on `/progress` across light/dark/sepia (e2e); every cell has an aria-label (page + memorized/strength/mistakes); juz bars + jump chips labelled; non-colour cues = page number + mistake dot. Grid is a roving-tabindex group (one Tab stop, arrow-navigable — see 4.3.2).
- [ ] **4.10.3** **Tests + budgets:** unit (store actions, reading-reward reducer, typed domain, parity), e2e (mark/persist/reload, bulk-mark, reading-reward accrual, strength + mistake coupling, weakest-pages nav, migration). `test:unit` + `test:e2e` green; `size` under budget; `build` clean; Progress absent from the reader chunk.
  - **Done:** unit **432** green, e2e **59** green — Progress: entry, 604/30-juz render, mark/persist/reload, bulk-mark, strength+hasanah, a11y×3 themes, migration render parity, weakest-chip → sheet → deep-link into reader, **roving-tabindex arrow nav**, **juz-jump scroll**; typed-domain + parity (4.2); `pageReviewData` (4.8.1); migration parity (4.10.1, integration + e2e). `size` under budget; `build` clean; Progress in its own chunk, absent from the reader bundle. *Reading-reward accrual stays unit-only (fake-clock) — a 90s wall-clock threshold isn't e2e-able; accepted.*
...
> ✅ **Phase 4 COMPLETE** (432 unit + 59 e2e green; type-check + build clean; size 54.16/120 KB JS, 12.63/30 KB CSS). All exit items met; both deferred polish items (grid roving-tabindex, juz-jump scroll) now landed. Only accepted non-item: reading-reward accrual is unit-only (a 90s wall-clock threshold isn't e2e-able).
