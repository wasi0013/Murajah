...
description: |
  ...
  (strip to essentials). Use when planning features, rethinking product direction,
  or pressure-testing whether you're building the right thing.
---
...
Search the codebase for `TODO`, `FIXME`, `HACK`, `XXX` markers. Read any existing architecture docs, README, or TODO files. When reading TODO files, specifically:
...
**STOP.** Ask the user which mode they want. Recommend one and explain WHY. Do NOT proceed until user responds.
...
**STOP.** For each issue, present the problem, your recommended fix, and 2-3 options. Do NOT proceed until user responds.
...
* Broad catch-all error handling is ALWAYS a smell. Name the specific exceptions.
...
Evaluate:
* N+1 queries. For every new database association traversal: is there a preload/eager load?
...
Identify at least 5 "bonus chunk" opportunities (<30 min each) that would make users think "oh nice, they thought of that." Present each as its own question. For each one, describe what it is, why it would delight users, and effort estimate. Then present options: **A)** Add to TODO list **B)** Skip **C)** Build it now.
